...
export function TidalPlayer({ track, onNext, onPrevious }: TidalPlayerProps) {
  ...
  // Tidal embed URL (requires API integration for full functionality)
  const tidalEmbedUrl = `https://embed.tidal.com/tracks/${track.providerId}?layout=gridify&disableAnalytics=true`

  const togglePlay = () => {
    // In a real implementation, this would control the Tidal player via API
    setIsPlaying(!isPlaying)
  }

  const toggleMute = () => {
    setIsMuted(!isMuted)
    // In a real implementation, this would control Tidal player volume
  }
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 w-full bg-zinc-900/90 backdrop-blur-md p-4 border-t border-zinc-800 shadow-lg">
      {/* Tidal Embed Player (hidden but functional) */}
      <iframe
        src={tidalEmbedUrl}
        width="0"
        height="0"
      ...
      <div className="flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto">
        <div className="flex items-center gap-4 mb-1">
          <Button
            variant="ghost"
            size="icon"
            className={cn("h-8 w-8 text-zinc-400 hover:text-white", isShuffle && "text-blue-500")}
          ...
          <Button
            variant="ghost"
            size="icon"
            className={cn("h-8 w-8 text-zinc-400 hover:text-white", isRepeat && "text-blue-500")}
        ...
        <div className="text-xs text-zinc-400">
          Tidal Player - Full integration requires Tidal API access
        </div>
      </div>
