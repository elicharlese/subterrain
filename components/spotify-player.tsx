...
export function SpotifyPlayer({ track, onNext, onPrevious }: SpotifyPlayerProps) {
  ...
  // For Spotify, we'll use their embed player
  // In a real implementation, you'd integrate with Spotify Web Playback SDK
  const spotifyEmbedUrl = `https://open.spotify.com/embed/track/${track.providerId}?utm_source=generator`

  const togglePlay = () => {
    // In a real implementation, this would control the Spotify player
    setIsPlaying(!isPlaying)
  }

  const toggleMute = () => {
    setIsMuted(!isMuted)
    // In a real implementation, this would control Spotify player volume
  }
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 w-full bg-zinc-900/90 backdrop-blur-md p-4 border-t border-zinc-800 shadow-lg">
      {/* Spotify Embed Player (hidden but functional) */}
      <iframe
        src={spotifyEmbedUrl}
        width="0"
        height="0"
      ...
      <div className="flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto">
        <div className="flex items-center gap-4 mb-1">
          <Button
            variant="ghost"
            size="icon"
            className={cn("h-8 w-8 text-zinc-400 hover:text-white", isShuffle && "text-green-500")}
          ...
          <Button
            variant="ghost"
            size="icon"
            className={cn("h-8 w-8 text-zinc-400 hover:text-white", isRepeat && "text-green-500")}
        ...
        <div className="text-xs text-zinc-400">
          Spotify Player - Full integration requires Spotify Premium
        </div>
      </div>
